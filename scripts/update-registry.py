    ...
    for raw in manual["chains"]:
        ...
        chains.append(
            Chain(
                ...
                average_blocktime_hint=raw["averageBlocktimeHint"],
                is_legacy=raw["isLegacy"],
                supports_shanghai=raw["supportsShanghai"],
                is_testnet=raw["isTestnet"],
                native_currency_symbol=raw.get(
                    "nativeCurrencySymbol", chainlist_native_currency_symbol(chainlist_entry)
                ),
                etherscan_api_url=raw["etherscanApiUrl"],
                etherscan_base_url=raw.get("etherscanBaseUrl", chainlist_explorer_url(chainlist_entry)),
                etherscan_api_key_name=raw["etherscanApiKeyName"],
...
def generated_named(chains: list[Chain]) -> str:
    ...
    for chain in chains:
        lines.append(f"    {chain.internal_id} = {chain.chain_id},")
    lines.extend(
        [
            "}",
            "",
            ...
            "",
            "impl NamedChain {",
            f"    /// The number of named chains.",
            f"    pub const COUNT: usize = {len(chains)};",
            "",
            "    /// All named chains in declaration order.",
            "    pub const VARIANTS: &'static [Self] = &[",
        ]
    )
    lines.extend(f"        Self::{chain.internal_id}," for chain in chains)
    lines.extend(
        [
            "    ];",
            "",
    ...
    lines.extend(f"        {rs_str(chain.name)}," for chain in chains)
    lines.extend(
        [
            "    ];",
            "",
    ...
    lines.extend(f"            {chain.chain_id} => Some(Self::{chain.internal_id})," for chain in chains)
    lines.extend(
        [
            "            _ => None,",
            "        }",
    ...
    lines.extend(f"            Self::{chain.internal_id} => {rs_str(chain.name)}," for chain in chains)
    lines.extend(
        [
            "        }",
            "    }",
    ...
    lines.extend(grouped_option_duration(chains))
    lines.extend(
        [
            "            _ => None,",
            "        }",
            ...
            f"        {matches_expr('self', [c for c in chains if c.is_legacy])}",
            "    }",
            "",
            "    /// Returns whether the chain supports the Shanghai hardfork.",
            "    #[inline]",
            "    pub const fn supports_shanghai(self) -> bool {",
            f"        {matches_expr('self', [c for c in chains if c.supports_shanghai])}",
            "    }",
            "",
            "    /// Returns whether the chain is a testnet.",
            "    #[inline]",
            "    pub const fn is_testnet(self) -> bool {",
            f"        {matches_expr('self', [c for c in chains if c.is_testnet])}",
            "    }",
            "",
    ...
    lines.extend(grouped_option_str(chains, lambda chain: chain.native_currency_symbol))
    lines.extend(
        [
            "            _ => None,",
            "        }",
    ...
    lines.extend(grouped_option_str(chains, lambda chain: chain.etherscan_api_key_name))
    lines.extend(
        [
            "            _ => None,",
            "        }",
    ...
    for chain in chains:
        names = serde_parse_names(chain)
        lines.append(f"            {match_patterns(names)} => Some(Self::{chain.internal_id}),")
    lines.extend(
        [
            "            _ => None,",
            "        }",
    ...
    for chain in chains:
        lines.append(f"            {match_patterns(parse_names(chain))} => Ok(Self::{chain.internal_id}),")
    lines.extend(
        [
    ...
    for chain in chains:
        for alias in chain.aliases:
            lines.append(f"    (NamedChain::{chain.internal_id}, {rs_str(alias)}),")
    lines.extend(
        [
            "];",
            "",
    ...
    for chain in chains:
        for alias in chain.serde_aliases:
            lines.append(f"    (NamedChain::{chain.internal_id}, {rs_str(alias)}),")
        if chain.serde_name is not None:
            lines.append(f"    (NamedChain::{chain.internal_id}, {rs_str(chain.serde_name)}),")
    lines.append("];")
    return "\n".join(lines) + "\n"
